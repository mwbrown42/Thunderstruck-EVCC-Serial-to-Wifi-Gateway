...
bool USBHostSerial::isConnected() const {
  return _connected;
}

int USBHostSerial::getNumDevices() const {
...
bool USBHostSerial::begin(int baud, int stopbits, int parity, int databits) {
  ...
  if (xTaskCreate(_USBHostSerial_task, "usb_dev_lib", 4096, this, 1, &_USBHostSerial_task_handle) == pdTRUE) {
    _log("[USBHost] USB Host stack setup completed");
    return true;
  }
  _log("[USBHost] USB Host stack setup failed");
  return false;
}
...
std::size_t USBHostSerial::write(const uint8_t *data, std::size_t len) {
  ...
  if (maxSize < len) {
    char buf[40];
    snprintf(buf, 40, "[USBHost] Buffer overflow: %u-%u", len, maxSize);
    _log(buf);
  } else {
...
bool USBHostSerial::_handle_rx(const uint8_t *data, size_t data_len, void *arg) {
  ...
  if (lenReceived < data_len) {
    static_cast<USBHostSerial*>(arg)->_log("[USBHost] RX buf overflow");
  }
  return true;
}
...
void USBHostSerial::_USBHostSerial_task(void *arg) {
  ...
  uint32_t lastWaitReportMs = 0;
  int lastReportedDevCount = -1;

  while (1) {
    uint32_t now = millis();
    int curDevCount = thisInstance->getNumDevices();

    if (curDevCount != lastReportedDevCount || (now - lastWaitReportMs >= 6000)) {
      lastWaitReportMs = now;
      lastReportedDevCount = curDevCount;
      if (curDevCount == 0) {
        thisInstance->_log("[USBHost] Waiting for EVCC cable... (0 devices detected on USB port. NOTE: DevKitC-1 requires 5V power to VBUS!)");
      } else {
        char buf[90];
        snprintf(buf, sizeof(buf), "[USBHost] Physical USB device detected (%d dev), opening VCP serial driver...", curDevCount);
    ...
    if (vcp == nullptr) {
      ...
      thisInstance->_log("[USBHost] *** EVCC USB-to-Serial adapter successfully OPENED! ***");
    }
    ...
    if (vcp->line_coding_set(&(thisInstance->_line_coding)) == ESP_OK) {
      thisInstance->_log("[USBHost] Line coding configured (9600 baud, 8N1). Ready for EVCC communication!");
    } else {
      thisInstance->_log("[USBHost] ERROR: Failed to set line coding (9600 baud)");
      continue;
    }
    ...
    while (1) {
      ...
      if (xSemaphoreTake(thisInstance->_device_disconnected_sem, 0) == pdTRUE) {
        ...
        thisInstance->_log("[USBHost] USB Device DISCONNECTED");
        break;
      }
      ...
      if (data && thisInstance->_connected) {
        esp_err_t tx_err = vcp->tx_blocking(data, pxItemSize, 1000);
        if (tx_err != ESP_OK) {
          thisInstance->_log("[USBHost] USB tx error");
        }
